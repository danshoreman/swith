...
if ( function_exists( 'add_theme_support' ) ) {
	...
	add_image_size( 'blog', 744, 302, true ); // blog
	add_image_size( 'blog-slider', 380, 170, true ); // blog - slider
...
function mfn_pagination()
{
	...
	$translate['next'] = mfn_opts_get('translate') ? mfn_opts_get('translate-next','Next page &rsaquo;') : __('Next page &rsaquo;','kora');
	$translate['prev'] = mfn_opts_get('translate') ? mfn_opts_get('translate-prev','&lsaquo; Prev page') : __('&lsaquo; Prev page','kora');
...
class New_Walker_Category extends Walker_Category {
	function start_el(&$output, $category, $depth, $args) {
		...
		if ( $use_desc_for_title == 0 || empty($category->description) )
			$link .= 'title="' . esc_attr( sprintf(__('View all posts filed under %s','kora'), $cat_name) ) . '"';
		else
...
function mfn_breadcrumbs() {
	...
	$translate['home'] = mfn_opts_get('translate') ? mfn_opts_get('translate-home','Home') : __('Home','kora');
	$translate['you-are-here'] = mfn_opts_get('translate') ? mfn_opts_get('translate-you-are-here','You are here:') : __('You are here:','kora');
...
// 	echo '<li>'. $translate['you-are-here'] .'</li>';
	...
	// Blog Category
	if ( is_category() ) {
		echo '<li><a href="'. curPageURL() .'">' . __('Archive by category','kora').' "' . single_cat_title('', false) . '"</a></li>';

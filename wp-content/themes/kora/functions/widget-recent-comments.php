			...
			$output = '<div class="Recent_comments">';
				$output .= '<ul>';
					foreach($comments as $comment)
					{
						...
						$comment_date = array();
						$comment_date['year'] = $date->format('Y');
						$comment_date['mounth'] = $date->format('M');
						$comment_date['day'] = $date->format('j');
						
						$output .= '<li>';
							$output .= '<span class="date"><span class="day">'. $comment_date['day'] .'</span><span class="mounth">'. $comment_date['mounth'] .'</span></span>';
							$output .= '<p><i class="icon-user"></i> <strong>'.strip_tags($comment->comment_author) .'</strong> <a href="'. $url .'">'. get_the_title($comment->comment_post_ID) .'</a></p>';
						$output .= '</li>';
					}
				$output .= '</ul>';
			$output .= '</div>'."\n";

...
function mfn_styles() 
{
	...
	$fonts['content'] = mfn_opts_get( 'font-content', 'PT Sans' );
	$fonts['menu'] = mfn_opts_get( 'font-menu', 'Open Sans' );
	$fonts['headings'] = mfn_opts_get( 'font-headings', 'Open Sans' );
	...
	wp_enqueue_style( 'Patua+One', 'http://fonts.googleapis.com/css?family=Patua+One');
}
...
function mfn_slider_config()
{
	...
	// What we offer
	$args_offer = array(
		'visible'	=> intval( mfn_opts_get( 'slider-offer-visible', 3 ) ),
		'auto'		=> round( intval( mfn_opts_get( 'slider-offer-auto' ) ) / 1000 ),
	);
	...
	echo 'window.mfn_slider_offer = { visible:'. (int)$args_offer['visible'] .', auto:'. (int)$args_offer['auto'] .' };'."\n";
